...
sealed class Loader(val id: String) {
		...
		override fun generateManifest(ctx: Context): String {
			val manifest = FabricManifest(
				...
				entrypoints = mapOf(
                    ...
                    "preLaunch" to listOf("${ctx.modGroup}.${ctx.modId}.platform.fabric.FabricClientPreLaunch"),
                    "modmenu" to listOf("${ctx.modGroup}.${ctx.modId}.platform.fabric.ModMenuIntegration"),
				),
				...
				suggests = ctx.extension.dependencies.optional.associate { it.modid.get() to it.fabricLikeVersionRange.get() },
	...
	sealed class ForgeLike(id: String) : Loader(id) {
		override val excludedResources = listOf(
            ...
            "audio_improvements.mixins.vinurl.json"
		)
