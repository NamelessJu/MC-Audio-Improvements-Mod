...
abstract class ModPlatformPlugin @Inject constructor() : Plugin<Project> {
	...
	private fun Project.configureProcessResources(ctx: Context) {
		...
		tasks.named<ProcessResources>("processResources") {
			...
			filesMatching(listOf("*.mixins*.json", "*.mixins*.json5")) {
	...
	private fun Project.configureFletchingTable(ctx: Context) {
		extensions.configure<FletchingTableExtension> {
			mixins.create("main") {
                mixin("default", "${ctx.modId}.mixins.json5")
                mixin("vinurl", "${ctx.modId}.mixins.vinurl.json")
            }
	...
	private fun Project.registerBuildAndCollectTask(ctx: Context) {
        val outputName = "${ctx.modName} v${ctx.modVersion} (${ctx.loader.javaClass.simpleName} ${ctx.currentMcVersion}).jar"
		tasks.register<Copy>("buildAndCollect") {
            ...
            */
			into(rootProject.layout.buildDirectory.dir("libs/${ctx.basicVersion}"))
			dependsOn("build")
			group = "build"
		}
	}
