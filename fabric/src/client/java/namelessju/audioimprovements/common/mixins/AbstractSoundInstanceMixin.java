...
public abstract class AbstractSoundInstanceMixin
{
    ...
    private Sound audioImprovements$getSound(WeighedSoundEvents weighedSoundEvents, RandomSource randomSource)
    {
        if (source == SoundSource.MUSIC)
        {
            ...
            {
                // no somewhat easy way to tell how many sounds are in the event
                // so unfortunately it'll just have to hit a max tries value
                int tries = 0;
                Sound sound;
                ...
                if (tries > 1)
                {
                    ...
                    {
                        AudioImprovements.LOGGER.info("Prevented playing repeat music \"{}\" x{}", mod.lastPlayedMusicLocation, tries - 1);
                    }
                    else AudioImprovements.LOGGER.info("Ran out of tries while trying to prevent repeat music \"{}\" (exceeded {} tries)", mod.lastPlayedMusicLocation, audioImprovements$MAX_REPEAT_PREVENTION_TRIES);
                }
