...
public abstract class AbstractSoundInstanceMixin
{
    ...
    private Sound redirectGetSound(WeighedSoundEvents weighedSoundEvents, RandomSource randomSource)
    {
        if (source == SoundSource.MUSIC)
        {
            AudioImprovements mod = AudioImprovements.instance();
            if (AudioImprovements.config().preventMusicRepeat && mod.lastPlayedMusic != null)
            {
                int tries = 0;
                Sound sound;
